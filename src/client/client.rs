...
use reqwest::{
    Client, StatusCode,
    header::{CONTENT_RANGE, HeaderMap, HeaderValue, RANGE, RETRY_AFTER},
};
use tokio_util::sync::CancellationToken;

use crate::{
    Error,
    client::retry::{RetryDecision, RetryPolicy},
    io_error,
};

pub type Verifier = Arc<dyn Fn(&Path) -> std::io::Result<bool> + Send + Sync>;
...
impl DownloadClient {
    pub fn new(http: Client, retry: RetryPolicy) -> Self {
        Self { http, retry }
    }
    ...
    pub async fn download(
        ...
        if let Some(parent) = destination.parent() {
            create_dir_all(parent).map_err(|e| io_error(parent, e))?;
        }
    ...
    async fn attempt(
        ...
        let mut existing_size = metadata(part_path).map(|m| m.len()).unwrap_or(0);

        if existing_size > expected_size {
            remove_file(part_path).map_err(|e| io_error(part_path, e))?;
            existing_size = 0;
        }
        ...
        if existing_size > 0 {
            headers.insert(
                RANGE,
                HeaderValue::from_str(&format!("bytes={existing_size}-")).unwrap(),
            );
        }
        ...
        while let Some(chunk) = stream.next().await {
            ...
            file.write_all(&bytes).map_err(|e| io_error(part_path, e))?;
        }

        file.flush().map_err(|e| io_error(part_path, e))?;
        drop(file);

        let actual_size = metadata(part_path).map(|m| m.len()).unwrap_or(0);
        ...
        let ok = verify(part_path).map_err(|e| io_error(part_path, e))?;
        if !ok {
            remove_file(part_path).map_err(|e| io_error(part_path, e))?;
        ...
        rename(part_path, destination).map_err(|e| io_error(destination, e))?;
...
fn file_matches(path: &Path, expected_size: u64, verify: &Verifier) -> std::io::Result<bool> {
